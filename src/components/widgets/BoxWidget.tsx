...
import "../../styles/widgets/box-widget.css";
import { BoxWidgetData } from "../../lib/DataType";

export function BoxWidget(props: {
  title: string;
  value: string;
  buttons: Array<BoxWidgetData>;
}) {
  return (
    <div className="box-widget">
      ...
      <p className="content">{props.value}</p>
        ...
        <div className="inner">
          {props.buttons.map((item, index) => (
            <button
              ...
              key={index}
              className={item.callback ? "" : "disabled"}
            >
              {item.icon}
            </button>
          ))}
